...
INCOME=`curl "$APIURL?do=check&serverip=$SERVERIP&type=2"`
echo $INCOME
...
echo before
qemu-img create -f qcow2 -b /opt/soda/openwrt/openwrt.qcow2 /opt/soda/machines/$UUID/disk.qcow2 ${TYPEDISK}M
echo after

echo screen
qemu-system-x86_64 \
                -name freeopenwrt-firstboot \
                -enable-kvm -cpu host \
                -drive file=/opt/soda/machines/$UUID/disk.qcow2,if=virtio \
                -m 245 -netdev type=user,id=net0,hostfwd=tcp::2229-:22 \
...
cat /opt/soda/openwrt/base.sh >> /opt/soda/machines/$UUID/start.sh
echo sshing
sshpass -p "defcon1337" scp -P2229 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no /opt/soda/machines/$UUID/start.sh root@127.0.0.1:/tmp/start.sh
retVal=$?
if [ $retVal -ne 0 ]; then
  curl \
  -H "Content-Type: application/json" \
...
sshpass -p "defcon1337" ssh -p2229 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@127.0.0.1 sh /tmp/start.sh
while true; do
MACHINE=`ps aux | grep qemu | grep "freeopenwrt-firstboot" | head -n 1`
if [ -z "$MACHINE" ];
then
break;
else
...
qemu-system-x86_64 \
                ...
                -enable-kvm -cpu host \
                -drive file=/opt/soda/machines/$UUID/disk.qcow2,if=virtio \
                -m 256 -smp 1 -daemonize -nodefaults -no-user-config  -nographic -device virtio-vga --sandbox on \
