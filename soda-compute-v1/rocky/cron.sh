...
INCOME=`curl "$APIURL?do=check&serverip=$SERVERIP&type=7"`
echo $INCOME
...
qemu-system-x86_64 \
                ...
                -m 2048 -netdev type=user,id=net0,hostfwd=tcp::2225-:22 \
...
echo sshing
sshpass -p "defcon1337" scp -P2225 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no /opt/soda/machines/$UUID/start.sh root@127.0.0.1:/tmp/start.sh
retVal=$?
if [ $retVal -ne 0 ]; then
  curl \
  -H "Content-Type: application/json" \
...
sshpass -p "defcon1337" ssh -p2225 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@127.0.0.1 sh /tmp/start.sh
while true; do
