...
INCOME=`curl "$APIURL?do=check&serverip=$SERVERIP&type=3"`
echo $INCOME
...
echo before
qemu-img create -f qcow2 -b /opt/soda/bsd/freebsd.qcow2 /opt/soda/machines/$UUID/disk.qcow2 ${TYPEDISK}M
echo after

echo screen
qemu-system-x86_64 \
                -name freebsd-firstboot \
                -enable-kvm -cpu host \
                -drive file=/opt/soda/machines/$UUID/disk.qcow2,if=virtio \
                -m 1024 -netdev type=user,id=net0,hostfwd=tcp::2223-:22 \
                -device virtio-net-pci,netdev=net0,mac=$NEWMAC \
                -daemonize --sandbox on -nodefaults -no-user-config -nographic -device virtio-vga
retVal=$?
if [ $retVal -ne 0 ]; then
  curl \
...
echo "NEWMASK=255.255.192.0" >> /opt/soda/machines/$UUID/start.sh
echo "NEWGATE=$NEWGATE" >> /opt/soda/machines/$UUID/start.sh
cat /opt/soda/bsd/base.sh >> /opt/soda/machines/$UUID/start.sh

sshpass -p "defcon1337!" scp -P2223 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no /opt/soda/machines/$UUID/start.sh root@127.0.0.1:/tmp/start.sh
retVal=$?
if [ $retVal -ne 0 ]; then
  curl \
  -H "Content-Type: application/json" \
...
sshpass -p "defcon1337!" ssh -p2223 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@127.0.0.1 sh /tmp/start.sh
while true; do
MACHINE=`ps aux | grep qemu | grep "freebsd-firstboot" | head -n 1`
if [ -z "$MACHINE" ];
then
break;
else
