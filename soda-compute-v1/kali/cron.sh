...
INCOME=`curl "$APIURL?do=check&serverip=$SERVERIP&type=8"`
echo $INCOME
...
echo before
qemu-img create -f qcow2 -b /opt/soda/kali/kali-base.qcow2 /opt/soda/machines/$UUID/disk.qcow2 ${TYPEDISK}M
echo after

echo screen
qemu-system-x86_64 \
                -name freekali-firstboot \
                -enable-kvm -cpu host \
                -drive file=/opt/soda/machines/$UUID/disk.qcow2,if=virtio \
                -m 4096 -netdev type=user,id=net0,hostfwd=tcp::2224-:22 \
...
cat /opt/soda/kali/base.sh >> /opt/soda/machines/$UUID/start.sh

sshpass -p "defcon1337" scp -P2224 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no /opt/soda/machines/$UUID/start.sh root@127.0.0.1:/tmp/start.sh
retVal=$?
if [ $retVal -ne 0 ]; then
  curl \
  -H "Content-Type: application/json" \
...
sshpass -p "defcon1337" ssh -p2224 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@127.0.0.1 sh /tmp/start.sh
while true; do
MACHINE=`ps aux | grep qemu | grep "freekali-firstboot" | head -n 1`
if [ -z "$MACHINE" ];
then
break;
else
